/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { MatchmakingRequestCreateNestedManyWithoutGamesInput } from "./MatchmakingRequestCreateNestedManyWithoutGamesInput";
import { Type } from "class-transformer";
import { TeamCreateNestedManyWithoutGamesInput } from "./TeamCreateNestedManyWithoutGamesInput";

@InputType()
class GameCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  genre?: string | null;

  @ApiProperty({
    required: false,
    type: () => MatchmakingRequestCreateNestedManyWithoutGamesInput,
  })
  @ValidateNested()
  @Type(() => MatchmakingRequestCreateNestedManyWithoutGamesInput)
  @IsOptional()
  @Field(() => MatchmakingRequestCreateNestedManyWithoutGamesInput, {
    nullable: true,
  })
  matchmakingRequests?: MatchmakingRequestCreateNestedManyWithoutGamesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  platform?: string | null;

  @ApiProperty({
    required: false,
    type: () => TeamCreateNestedManyWithoutGamesInput,
  })
  @ValidateNested()
  @Type(() => TeamCreateNestedManyWithoutGamesInput)
  @IsOptional()
  @Field(() => TeamCreateNestedManyWithoutGamesInput, {
    nullable: true,
  })
  teams?: TeamCreateNestedManyWithoutGamesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  title?: string | null;
}

export { GameCreateInput as GameCreateInput };
