/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { MatchmakingRequestUpdateManyWithoutGamesInput } from "./MatchmakingRequestUpdateManyWithoutGamesInput";
import { Type } from "class-transformer";
import { TeamUpdateManyWithoutGamesInput } from "./TeamUpdateManyWithoutGamesInput";

@InputType()
class GameUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  genre?: string | null;

  @ApiProperty({
    required: false,
    type: () => MatchmakingRequestUpdateManyWithoutGamesInput,
  })
  @ValidateNested()
  @Type(() => MatchmakingRequestUpdateManyWithoutGamesInput)
  @IsOptional()
  @Field(() => MatchmakingRequestUpdateManyWithoutGamesInput, {
    nullable: true,
  })
  matchmakingRequests?: MatchmakingRequestUpdateManyWithoutGamesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  platform?: string | null;

  @ApiProperty({
    required: false,
    type: () => TeamUpdateManyWithoutGamesInput,
  })
  @ValidateNested()
  @Type(() => TeamUpdateManyWithoutGamesInput)
  @IsOptional()
  @Field(() => TeamUpdateManyWithoutGamesInput, {
    nullable: true,
  })
  teams?: TeamUpdateManyWithoutGamesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  title?: string | null;
}

export { GameUpdateInput as GameUpdateInput };
